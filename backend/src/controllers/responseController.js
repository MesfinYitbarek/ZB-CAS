...
exports.submitSupervisorEvaluation = asyncHandler(async (req, res, next) => {
  ...
  if (assessment.type === 'SupervisorOnly') {
    result = await scoringService.scoreIndividual(assessmentId, employeeId);
...
exports.getProgress = asyncHandler(async (req, res, next) => {
  ...
  const submitted = await Response.findOne({ assessmentId, userId: req.user.id, respondentType: 'self', submittedAt: { $ne: null } }).lean();
