  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-card overflow-hidden">
      <div className="flex items-center gap-2.5 px-4 py-2 border-b border-gray-100 bg-gray-50">
        <div className={`w-7 h-7 rounded-xl flex items-center justify-center flex-shrink-0 ${isTop ? 'bg-gray-200' : 'bg-red-100'}`}>
          {isTop ? <Medal className="w-3.5 h-3.5 text-gray-700" /> : <AlertTriangle className="w-3.5 h-3.5 text-red-500" />}
        </div>
        <div>
          <h3 className="text-[13px] font-bold text-gray-900">{title}</h3>
          <p className="text-[11px] text-gray-400">Top {data.length} by average score</p>
        </div>
      </div>
      <div className="grid grid-cols-[24px_1fr_48px_64px_32px] gap-2 px-4 py-1.5 bg-gray-50 border-b border-gray-100">
        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">#</span>
        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Name</span>
        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide text-center">Results</span>
        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide text-right">Avg Score</span>
        <span />
      </div>
      <div className="divide-y divide-gray-100">
        {data.length === 0 ? (
          <p className="text-[13px] text-gray-400 text-center py-6">No data available</p>
        ) : data.map((row, i) => (
          <div key={i} className="grid grid-cols-[24px_1fr_48px_64px_32px] gap-2 px-4 py-1.5 items-center hover:bg-gray-50 transition-colors group">
