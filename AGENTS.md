...
- **Assessment lifecycle:** `DRAFT → SCHEDULED → ACTIVE → COMPLETED` (COMPLETED is terminal; the legacy `ARCHIVED` status was removed — existing archived assessments were converted to COMPLETED). Auto-transitions driven by `node-cron` scheduler (`backend/src/services/schedulerService.js`). Admins can extend an ACTIVE assessment's deadline via `PATCH /api/assessments/:id/deadline` (`extendDeadline` in `assessmentController.js` — ACTIVE-only, re-arms timers via `clearAssessmentTimers` + `scheduleAssessmentTimers`); the Edit-Assessment form is DRAFT-only.
...
- **Migration notes:** `src/models/*.js` are dead Mongoose code — no active file imports them (verified). Backend serializes Prisma rows with an `_id` alias (`{...row, _id: row.id}`) so the frontend needs no changes. The legacy `Report`/`ReportCompetency` snapshot tables are still written only by the demo seed (`seed/seedBulkReports.js`) — no production code reads them; the production writer `reportService.js` is dead code. Competency target groups live in the `CompetencyTargetGroup` junction, and assessment questions in the `AssessmentQuestion` junction. **Assessment target audience:** there is NO `audience` field/relation on the Prisma `Assessment` model (it was a Mongoose subdocument). Use the scalar `audienceType` (`ALL_DEPARTMENTS`/`DEPARTMENT_ALL`/`SPECIFIC_EMPLOYEES`), the 1:1 `targetAudience` relation (`AssessTargetAudience` with a `type` field), and the direct relations `audienceDepartments` / `audienceEmployees` junctions. Filters must query those direct relations (e.g. `{ audienceDepartments: { some: { department } } }`), NOT an `audience` wrapper — Prisma throws "unknown argument 'audience'".

## Reports Module (two-tier, live-first as of this session)

- **Tier 1 — LIVE analytics.** `GET /api/reports/stats`, `/heatmap`, `/department/:department`, `/filter-options` are now handled by `analyticsController.js` → `services/analyticsService.js`, computed **on-request from the live `Result` table, scoped to `status: 'COMPLETED'` assessments and `status: 'FINAL'` results ONLY** (the COMPLETED gate is enforced in `buildResultFilter`). No snapshot/report tables involved — high-level dashboards are always current. `scoreMin`/`scoreMax`/`overallLevel` filters are applied AFTER grouping (per user×assessment mean), in `computeLiveStats`. Levels use the single `assignLevel` (80/60/40) from `utils/scoring.js`.
- **Tier 2 — named on-demand reports (Excel only, live-sourced).** `POST /api/reports/generate` (+ `/generated`, `/generated/:id`, `/generated/:id/download`, `DELETE /generated/:id`) are handled by `generatedReportController.js` → `services/generatedReportService.js`. Every report is a `GeneratedReport` row (`title`, `description`, `type` INDIVIDUAL/ORGANIZATION/CUSTOM_PIVOT, frozen `filters` + `pivot` JSON, `fileName`, `rowCount`, `generatedBy`) with the rendered `.xlsx` stored under `backend/storage/reports/`. Datasets always come from the **live `Result` table** via `buildResultFilter` + `fetchLiveResults` with the same per-(user×assessment) mean gate Tier 1 uses for `scoreMin`/`scoreMax`/`overallLevel`. **PDF generation is fully removed** (pdfkit dep dropped from backend; jsPDF/`exportToPDF` removed from frontend — `exportToExcel` in `utils/exportUtils.js` stays for Activity Log). The old snapshot-read surface (`/custom/pivot`, `/export/*`, `/individual/:userId`, `/`, `/:reportId`, and the PDF builders + stale 85/70/50 `assignLevel` in `reportController.js`) is deleted; `reportController.js` now only serves `/employees`. Frontend has one unified page `pages/reports/generated/GeneratedPage.jsx` (+ `NewReportModal`, `GeneratedDetailModal`); the old Individual/All/Builder tabs, `ReportTables`, `ReportDetailModal` are deleted (`EmployeePicker` is reused by the new-report form). Heavy-report async queue (ReportJob + worker) is still a future upgrade — generation is synchronous today.
